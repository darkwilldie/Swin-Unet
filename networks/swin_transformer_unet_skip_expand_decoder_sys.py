...
from networks.as_mlp import BasicLayer_mlp, MyNorm
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.layers_as_mlp = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer_as_mlp = BasicLayer_mlp(dim=int(embed_dim * 2 ** i_layer),
                                 input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                     patches_resolution[1] // (2 ** i_layer)),
                                 depth=depths[i_layer],
                                 shift_size=5,
                                 ...
                                 drop=drop_rate,
                                 drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],
                                 norm_layer=MyNorm if self.patch_norm else None,
                                 downsample=None,
                                 use_checkpoint=use_checkpoint)
            self.layers_as_mlp.append(layer_as_mlp)
    ...
    def forward_features(self, x):
        ...
        assert len(self.layers_as_mlp) == len(self.layers)
        ...
        return x, x_downsample, x_axialShifted

    #Dencoder and Skip connection
    def forward_up_features(self, x, x_downsample, x_axialShifted):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)
            else:
                x = torch.cat([x, x_downsample[3-inx], x_axialShifted[3-inx]], -1)
    ...
    def forward(self, x):
        x, x_downsample, x_axialShifted = self.forward_features(x)
        x = self.forward_up_features(x,x_downsample, x_axialShifted)
        x = self.up_x4(x)
